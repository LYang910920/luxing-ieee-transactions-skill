...
The profile has `expanded-partial-calibration`: 73 verified full texts were deep-read for research architecture and correction doctrine, comprising 42 owner-provided papers and 31 nonduplicate open-access journal articles. Sentence-level style remains calibrated from the role-weighted local corpus, including 20 first/corresponding-author core-voice records and 17 IEEE Transactions papers. Corresponding status is a useful authorship signal but does not prove section-level drafting ownership; many contribution roles remain unconfirmed. The package records lawful metadata, aggregate counts, derived metrics, rules, and paper cards but no article full text or per-paper deep-reading notes. All 17 owner-priority selected Transactions rows have private full text; one different-subfield row is explicitly owner-deprioritized. The broader publication universe, section-level voice attribution, venue-specific microstyle, and publisher-final verification of the 2026 TDSC-matched manuscript remain open; do not use `full-corpus-calibrated` until the full audit protocol passes.
